    ...
    for (int i = 0; i < file->service_count(); ++i) {
        ...
        for (int j = 0; j < s->method_count(); ++j) {
            ...
            xc_ << "    bool send_" << m->name() << "(const " << m->input_type()->name() << "& req) {\n"
                ...
                << "        if (!this->send_request(ProcNumber::" << m->name() << ", seq_++, req, true)){ this->disconnect(); return false; }\n"
                << "        return true;\n"
                << "    }\n";
...
void nbcg::generateXS(const gp::FileDescriptor* file) const {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
        ...
        xs_ << "template <typename T, bool NB_DEFAULT = false";
        for (int j = 0; j < s->method_count(); ++j)
            xs_ << ", bool NB_" << up(s->method(j)->name()) << " = NB_DEFAULT";
        xs_ << ">\n";
        
        xs_ << "struct " << s->name() << "Interface : public rpc::rpc_server_base<T> {\n"
